...
    // NOTE: We store the YouTube Channel Name in the 'description' field for leaders' videos.
...
// ============================================================
// 🔹 5. Get Videos (Internal helper function - MODIFIED)
// ============================================================
...
    const offset = (page - 1) * limit;
    ...
    // ✅ Naya: channelName query parameter nikal lein
    const { category, channelName } = req.query; 
    
    const options = {
...
    }
    ...
    // ✅ Channel Name filter: Leader Video ko 'description' field se filter karein
    if (Model === db.LeaderVideo && channelName && channelName !== 'All') {
        options.where.description = channelName; 
    }
...
  }
  // NOTE: Leader Videos mein 'description' field Channel Name ke liye istemaal ho raha hai
  // isliye yahan koi 'category' logic nahi hai.
...
// ============================================================
// 🔹 9. Get Leader Channels (✅ NAYA FUNCTION: Unique Channel Names)
// ============================================================
const getLeaderChannels = asyncHandler(async (req, res) => {
    
    const channels = await db.LeaderVideo.findAll({
       attributes: ['description'], // Channel name 'description' column में store है
       group: ['description'], 
       where: {
           description: { [Op.ne]: null, [Op.ne]: '' } // Empty ya null channel name ko ignore karein
       },
       order: [['description', 'ASC']]
  });

  const channelList = channels.map(item => item.description);
  
  res.status(200).json({ success: true, data: channelList });
});
...
exports.getLeaderChannels = getLeaderChannels; // ✅ Naya export
